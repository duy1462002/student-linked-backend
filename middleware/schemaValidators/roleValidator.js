...
function canAddRemoveMemberRole(userRole) {    
...
exports.addRemoveMember = (req, res, next) => {
    ...
    if (req.body && req.body.userId && req.body.roles && req.body.roles.length > 0) {
        return UserRoles.findOne({ user: req.userData.userId }).then(role => {
            if(canAddRemoveMemberRole(role)){
                console.log("OKOK");
                next();
                return;
            }else{
                return res.status(400).json({ message: "Does not have permission to add user role!" });    
            }
        }).catch(err=>{
...
exports.getRolesOfUser = (req, res, next) => {
