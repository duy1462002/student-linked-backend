...
function addProfilePicture(postData) {
  return (dispatch) => {
    dispatch(request());

    postService.addProfilePicture(postData).then(
      (res) => {
...
function likePost(postId, authorId, postLikes) {
  return (dispatch) => {
    ...
    postService.likePost(postId, authorId).then(
      () => {},
      (error) => {
        console.log(error);
      }
