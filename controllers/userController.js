        ...
        if (err) {
          ...
          user
            ...
            .then((user) => {
              ...
              Roles.findOne({ code: roleConstant.REGISTERED_MEMBER }).then(role => {

                let roleId = role.id;
                const userRoles = new UserRoles({ user: user._id, roles: [{ role: roleId }] }).save();
    ...
    .then((users) => {
      if (users.length < 1) {
        ...
        bcrypt.compare(req.body.password, users[0].password, (err, result) => {
          ...
          return res.status(400).json({ message: "2003-Incorrect credentials." });
        });
      }
    })
    .catch((err) => {
